...
class SettingsTab(tk.Frame):
    ...
    def _build(self):
        _section_header(self, "설정",
                         "API 키는 EXE와 같은 폴더의 tubemetric_keys.json 에 저장됩니다.")

        wrap = tk.Frame(self, bg=BG, padx=36)
        wrap.pack(fill="both", expand=True)

        # ── YouTube API Key ────────────────────────────────────────────────────
        tk.Frame(wrap, bg=BG, height=20).pack()
        tk.Label(wrap, text="YouTube Data API v3 Key",
                 font=("Arial", 11, "bold"), bg=BG, fg=FG, anchor="w").pack(fill="x")
        tk.Label(wrap,
                 text="Google Cloud Console → API 및 서비스 → 사용자 인증 정보에서 발급",
                 font=("Arial", 8), bg=BG, fg=FG_DIM, anchor="w").pack(fill="x", pady=(2, 6))

        yt_border = tk.Frame(wrap, bg=ACCENT, padx=1, pady=1)
        yt_border.pack(fill="x")
        self._yt_var = tk.StringVar()
        self._yt_entry = tk.Entry(
            yt_border, textvariable=self._yt_var,
            font=("Consolas", 11), bg=BG3, fg=FG,
            insertbackground=ACCENT, relief="flat", show="",
        )
        self._yt_entry.pack(fill="x", ipady=10, ipadx=10)

        self._yt_status = tk.StringVar()
        tk.Label(wrap, textvariable=self._yt_status,
                 font=("Arial", 8), bg=BG, fg=FG_DIM, anchor="w").pack(fill="x", pady=(4, 0))

        # ── GitHub Token ───────────────────────────────────────────────────────
        tk.Frame(wrap, bg=BG, height=24).pack()
        tk.Label(wrap, text="GitHub Personal Access Token",
                 font=("Arial", 11, "bold"), bg=BG, fg=FG, anchor="w").pack(fill="x")
        tk.Label(wrap,
                 text="GitHub → Settings → Developer settings → Personal access tokens (repo 권한 필요)",
                 font=("Arial", 8), bg=BG, fg=FG_DIM, anchor="w").pack(fill="x", pady=(2, 6))

        gh_border = tk.Frame(wrap, bg=ACCENT, padx=1, pady=1)
        gh_border.pack(fill="x")
        self._gh_var = tk.StringVar()
        self._gh_entry = tk.Entry(
            gh_border, textvariable=self._gh_var,
            font=("Consolas", 11), bg=BG3, fg=FG,
            insertbackground=ACCENT, relief="flat", show="●",
        )
        self._gh_entry.pack(fill="x", ipady=10, ipadx=10)

        # GitHub Repo
        tk.Frame(wrap, bg=BG, height=12).pack()
        tk.Label(wrap, text="GitHub 저장소  (owner/repo)",
                 font=("Arial", 10, "bold"), bg=BG, fg=FG, anchor="w").pack(fill="x")
        repo_border = tk.Frame(wrap, bg=BORDER, padx=1, pady=1)
        repo_border.pack(fill="x", pady=(4, 0))
        self._repo_var = tk.StringVar()
        tk.Entry(
            repo_border, textvariable=self._repo_var,
            font=("Consolas", 11), bg=BG3, fg=FG,
            insertbackground=ACCENT, relief="flat",
        ).pack(fill="x", ipady=8, ipadx=10)

        self._gh_status = tk.StringVar()
        tk.Label(wrap, textvariable=self._gh_status,
                 font=("Arial", 8), bg=BG, fg=FG_DIM, anchor="w").pack(fill="x", pady=(4, 0))

        # ── 토글 보기/숨기기 ─────────────────────────────────────────────────────
        self._show_gh = False
        toggle_row = tk.Frame(wrap, bg=BG, pady=6)
        toggle_row.pack(fill="x")
        self._toggle_btn = _btn(toggle_row, "GitHub 토큰 보기", self._toggle_gh, padx=10, pady=5)
        self._toggle_btn.pack(side="left")

        # ── 저장 버튼 ─────────────────────────────────────────────────────────
        tk.Frame(wrap, bg=BG, height=16).pack()
        save_btn = _btn(wrap, "저장", self._save,
                        bg=ACCENT, fg="white", bold=True, padx=28, pady=12)
        save_btn.pack(anchor="w")

        self._msg_var = tk.StringVar()
        tk.Label(wrap, textvariable=self._msg_var,
                 font=("Arial", 9, "bold"), bg=BG, fg=GREEN, anchor="w").pack(fill="x", pady=(8, 0))

    def _toggle_gh(self):
        self._show_gh = not self._show_gh
        self._gh_entry.configure(show="" if self._show_gh else "●")
        self._toggle_btn.configure(text="GitHub 토큰 숨기기" if self._show_gh else "GitHub 토큰 보기")

    def _load(self):
        """저장된 키와 현재 활성 키를 불러와 입력란에 표시."""
        keys = _load_keys()
        self._yt_var.set(keys.get("youtube_api_key", ""))
        self._gh_var.set(keys.get("github_token", ""))
        # repo: 저장된 값 → config.py 기본값 순
        repo = keys.get("github_repo", "")
        if not repo:
            try:
                ...
                pass
        self._repo_var.set(repo)
        self._refresh_status()

    def _refresh_status(self):
        yt = _load_yt_api_key()
        if yt:
            self._yt_status.set(f"✓  현재 적용 중: {yt[:12]}...")
        else:
            self._yt_status.set("✗  YouTube API Key 없음")

        gh_token, gh_repo = _load_credentials()
        if gh_token and gh_repo:
            self._gh_status.set(f"✓  현재 적용 중: {gh_token[:8]}...  ({gh_repo})")
        else:
            self._gh_status.set("✗  GitHub 토큰 없음")

    def _save(self):
        yt_key  = self._yt_var.get().strip()
        gh_tok  = self._gh_var.get().strip()
        gh_repo = self._repo_var.get().strip()

        if not yt_key and not gh_tok:
            messagebox.showwarning("입력 필요", "API 키를 하나 이상 입력하세요.")
            return

        _save_keys(yt_key, gh_tok, gh_repo)
        self._refresh_status()
        self._msg_var.set("✓  저장 완료  —  지금 바로 적용됩니다.")
        self.after(4000, lambda: self._msg_var.set(""))
