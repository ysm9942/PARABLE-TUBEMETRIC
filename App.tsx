...
const App: React.FC = () => {
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-[#05050e] text-white flex items-center justify-center p-6 selection:bg-violet-500/30">
        <div className="w-full max-w-md space-y-10 animate-in fade-in duration-500">
          <div className="text-center space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#9494b8] text-sm">Enter your PIN to continue</p>
            </div>
          </div>
          ...
          <form onSubmit={handlePinSubmit} className="space-y-4">
            <div className="relative">
              <input
                ...
                autoFocus
                className="w-full bg-[#0c0c22] border border-[#ffffff16] rounded-xl py-4 px-6 text-center text-2xl font-medium tracking-[0.4em] text-white focus:outline-none focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/20 transition-all placeholder:text-[#404060] placeholder:tracking-normal placeholder:text-base"
              />
            </div>
            <button
              type="submit"
          ...
          <p className="text-center text-xs text-[#c8c8e8]">Authorized access only</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#05050e] text-white flex font-sans overflow-hidden selection:bg-violet-500/30">

      {/* Modal: Instagram User Details */}
      {selectedIgUser && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0c0c22] w-full max-w-4xl max-h-[85vh] rounded-2xl border border-[#ffffff16] overflow-hidden flex flex-col shadow-md">
            <div className="p-5 border-b border-white/[0.11] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[13px] text-[#7878a8] mt-1 flex items-center gap-3">
                    ...
                    <span>평균 댓글 <span className="text-[#9494b8] font-medium">{selectedIgUser.avgComments.toLocaleString()}</span></span>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedIgUser(null)} className="p-2 hover:bg-white/8 rounded-lg transition-colors text-[#9494b8] hover:text-white"><X size={18} /></button>
            </div>
            <div className="overflow-y-auto flex-1">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px] sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Reel</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Views</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Likes</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Comments</th>
                ...
                <tbody className="divide-y divide-[#ffffff0c]">
                  {selectedIgUser.reels.map((reel, i) => (
                    <tr key={reel.media_pk || i} className="hover:bg-white/[0.05] transition-colors">
                      <td className="px-5 py-3 flex items-center gap-3">
                        {reel.thumbnail_url ? (
                          <img src={reel.thumbnail_url} className="w-9 h-14 object-cover rounded-lg border border-[#ffffff16] shrink-0" />
                        ) : (
                          <div className="w-9 h-14 bg-[#0a0a18] rounded-lg flex items-center justify-center shrink-0"><Instagram size={14} className="text-[#c8c8e8]" /></div>
                        )}
                        <span className="text-xs text-[#c8c8e8] line-clamp-2 max-w-[260px]">{reel.caption_text || '(캡션 없음)'}</span>
                      </td>
                      <td className="px-5 py-3 text-right text-sm font-semibold text-pink-400 tabular-nums">{reel.view_count.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right text-[13px] text-violet-400 tabular-nums">{reel.like_count.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right text-[13px] text-[#c8c8e8] tabular-nums">{reel.comment_count.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#9494b8]">{reel.video_duration ? `${Math.round(reel.video_duration)}s` : '—'}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#c8c8e8] font-mono">{reel.taken_at ? new Date(reel.taken_at).toLocaleDateString('ko-KR') : '—'}</td>
                      <td className="px-5 py-3 text-center">
                        {reel.url ? (
                          <a href={reel.url} target="_blank" className="p-1.5 bg-white/[0.07] hover:bg-pink-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all inline-flex"><ExternalLink size={13} /></a>
                        ) : '—'}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0c0c22] w-full max-w-4xl max-h-[85vh] rounded-2xl border border-[#ffffff16] overflow-hidden flex flex-col shadow-md">
            <div className="p-5 border-b border-white/[0.11] flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setSelectedLiveCreator(null)} className="p-2 hover:bg-white/8 rounded-lg transition-colors text-[#9494b8] hover:text-white"><X size={18} /></button>
            </div>
            <div className="overflow-y-auto flex-1">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px] sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">방송 제목</th>
                    <th className="px-5 py-3 text-center font-medium">카테고리</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">평균 시청자</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">최고 시청자</th>
                ...
                <tbody className="divide-y divide-[#ffffff0c]">
                  {selectedLiveCreator.streams.map((s, i) => (
                    <tr key={i} className="hover:bg-white/[0.05] transition-colors">
                      <td className="px-5 py-3.5 text-[13px] text-[#c8c8e8] max-w-[260px] truncate">{s.title || '(제목 없음)'}</td>
                      <td className="px-5 py-3 text-center">
                        <span className="text-[10px] bg-white/[0.07] px-2 py-0.5 rounded text-[#9494b8] border border-[#ffffff16]">{s.category || '—'}</span>
                      </td>
                      <td className="px-5 py-3 text-right text-sm font-semibold text-orange-400 tabular-nums">{s.avgViewers.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right text-[13px] text-red-400 tabular-nums font-medium">{s.peakViewers.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#9494b8]">{s.durationMin ? `${Math.floor(s.durationMin / 60)}h ${s.durationMin % 60}m` : '—'}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#9494b8] font-mono">{s.date || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0c0c22] w-full max-w-4xl max-h-[85vh] rounded-2xl border border-[#ffffff16] overflow-hidden flex flex-col shadow-md">
            <div className="p-5 border-b border-white/[0.11] flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setSelectedTkUser(null)} className="p-2 hover:bg-white/8 rounded-lg transition-colors text-[#9494b8] hover:text-white"><X size={18} /></button>
            </div>
            <div className="overflow-y-auto flex-1">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px] sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Video</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Views</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Likes</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Comments</th>
                    <th className="px-5 py-3 text-center font-medium">Duration</th>
                ...
                <tbody className="divide-y divide-[#ffffff0c]">
                  {selectedTkUser.videos.map((v, i) => (
                    <tr key={v.id || i} className="hover:bg-white/[0.05] transition-colors">
                      <td className="px-5 py-3 flex items-center gap-3">
                        {v.thumbnail ? (
                          <img src={v.thumbnail} className="w-9 h-14 object-cover rounded-lg border border-[#ffffff16] shrink-0" />
                        ) : (
                          <div className="w-9 h-14 bg-[#0a0a18] rounded-lg flex items-center justify-center shrink-0"><Music size={14} className="text-[#c8c8e8]" /></div>
                        )}
                        <span className="text-xs text-[#c8c8e8] line-clamp-2 max-w-[260px]">{v.title || '(제목 없음)'}</span>
                      </td>
                      <td className="px-5 py-3 text-right text-sm font-semibold text-cyan-400 tabular-nums">{v.viewCount.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right text-[13px] text-pink-400 tabular-nums">{v.likeCount.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right text-[13px] text-[#c8c8e8] tabular-nums">{v.commentCount.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#9494b8]">{v.duration ? `${v.duration}s` : '—'}</td>
                      <td className="px-5 py-3.5 text-center text-[13px] text-[#c8c8e8] font-mono">{v.uploadDate ? `${v.uploadDate.slice(0,4)}-${v.uploadDate.slice(4,6)}-${v.uploadDate.slice(6,8)}` : '—'}</td>
                      <td className="px-5 py-3 text-center">
                        {v.url ? (
                          <a href={v.url} target="_blank" className="p-1.5 bg-white/[0.07] hover:bg-cyan-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all inline-flex"><ExternalLink size={13} /></a>
                        ) : '—'}
                      </td>
                    </tr>
                  ))}
      ...
      {selectedChannel && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0c0c22] w-full max-w-6xl h-[85vh] rounded-2xl border border-[#ffffff16] overflow-hidden flex flex-col shadow-md animate-in fade-in duration-200">
            <div className="p-6 border-b border-white/[0.11] flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img src={selectedChannel.thumbnail} className="w-12 h-12 rounded-xl border border-[#ffffff1e] object-cover" alt="" />
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                    ...
                    <a href={`https://youtube.com/channel/${selectedChannel.channelId}`} target="_blank" className="text-[#9494b8] hover:text-violet-400 transition-all"><ExternalLink size={16} /></a>
                  </h3>
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <p className="text-xs text-[#9494b8]">Analytics Results</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button onClick={() => setSelectedChannel(null)} className="p-2 bg-white/[0.07] hover:bg-white/[0.14] text-[#c8c8e8] hover:text-white rounded-lg transition-all">
                  <X size={20} />
                </button>
              </div>
            </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between border-b border-white/[0.11] pb-3">
                    <h4 className="text-sm font-semibold text-white flex items-center gap-2">
                      ...
                      Shorts <span className="text-[#9494b8] font-normal">({selectedChannel.shortsList.length})</span>
                    </h4>
                    <div className="text-right">
                      <div className="text-xs text-[#9494b8] mb-0.5">Avg Views</div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {selectedChannel.shortsList.map((v) => (
                      <div key={v.id} className="bg-white/[0.07] p-3 rounded-xl border border-[#ffffff16] flex items-center gap-4 hover:bg-white/[0.08] hover:border-violet-400/40 transition-all group">
                        <img src={v.thumbnail} className="w-12 h-12 rounded-lg object-cover" alt="" />
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-[#c8c8e8] truncate leading-snug group-hover:text-white">{v.title}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs text-violet-400 font-medium">{v.viewCount.toLocaleString()} views</span>
                            <span className="text-xs text-[#c8c8e8]">{new Date(v.publishedAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                        <a href={`https://youtube.com/shorts/${v.id}`} target="_blank" className="p-2 bg-white/[0.07] text-[#c8c8e8] hover:text-white hover:bg-violet-600 rounded-lg transition-all">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between border-b border-white/[0.11] pb-3">
                    <h4 className="text-sm font-semibold text-white flex items-center gap-2">
                      ...
                      Longform <span className="text-[#9494b8] font-normal">({selectedChannel.longsList.length})</span>
                    </h4>
                    <div className="text-right">
                      <div className="text-xs text-[#9494b8] mb-0.5">Avg Views</div>
                      <div className="text-base font-semibold text-[#c8c8e8]">{selectedChannel.avgLongViews.toLocaleString()}</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-3">
                    {selectedChannel.longsList.map((v) => (
                      <div key={v.id} className="bg-white/[0.07] p-3 rounded-xl border border-[#ffffff16] flex items-center gap-4 hover:bg-white/[0.08] hover:border-white/20 transition-all group">
                        <img src={v.thumbnail} className="w-20 h-12 rounded-lg object-cover" alt="" />
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-[#c8c8e8] truncate leading-snug">{v.title}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs text-[#c8c8e8] font-medium">{v.viewCount.toLocaleString()} views</span>
                            <span className="text-xs text-[#c8c8e8]">{new Date(v.publishedAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                        <a href={`https://youtube.com/watch?v=${v.id}`} target="_blank" className="p-2 bg-white/[0.07] text-[#c8c8e8] hover:text-white hover:bg-[#1e1e34] rounded-lg transition-all">
                          <ExternalLink size={14} />
                        </a>
                      </div>
                    ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0c0c22] w-full max-w-6xl h-[85vh] rounded-2xl border border-[#ffffff16] overflow-hidden flex flex-col shadow-md animate-in fade-in duration-200">
            <div className="p-6 border-b border-white/[0.11] flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img src={selectedAdResult.thumbnail} className="w-12 h-12 rounded-xl border border-[#ffffff1e] object-cover" alt="" />
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <p className="text-xs text-[#9494b8]">Ad Detection Details</p>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedAdResult(null)} className="p-2 bg-white/[0.07] hover:bg-white/[0.14] text-[#c8c8e8] hover:text-white rounded-lg transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-5">
              {selectedAdResult.adVideos.length === 0 ? (
                <div className="h-full flex items-center justify-center text-[#c8c8e8] text-sm">분석된 광고 영상이 없습니다.</div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedAdResult.adVideos.map((v) => (
                    <div key={v.id} className="bg-white/[0.07] p-5 rounded-xl border border-[#ffffff16] hover:border-violet-400/40 transition-all group">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-2">
                              <span className="text-xs text-[#c8c8e8] flex items-center gap-1"><Eye size={11}/> {v.viewCount.toLocaleString()}</span>
                              <span className="text-xs text-[#9494b8] flex items-center gap-1"><ThumbsUp size={11}/> {v.likeCount.toLocaleString()}</span>
                            </div>
                          </div>

                          <div className="mt-3 pt-3 border-t border-white/[0.11] space-y-1.5">
                             <div className="text-xs text-[#c8c8e8]">Detection Evidence</div>
                      ...
                      <div className="mt-4 flex items-center justify-between">
                         <div className="text-xs text-[#c8c8e8]">Published: {new Date(v.publishedAt).toLocaleDateString()}</div>
      ...
      {/* Sidebar Navigation */}
      <aside className="w-[220px] bg-[#08081a] border-r border-[#ffffff1a] flex flex-col shrink-0 hidden xl:flex h-screen overflow-y-auto">
        ...
        <div className="px-5 py-4 border-b border-[#ffffff1a]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[13px] font-bold text-white tracking-tight leading-none">TubeMetric</div>
              <div className="text-[10px] text-[#6b6b8f] leading-none mt-0.5">by Parable</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-6 overflow-y-auto">
          ...
          {/* ANALYSIS */}
          <div>
            <p className="px-2 mb-1.5 text-[9px] font-semibold text-[#4e4e6a] tracking-[0.12em] uppercase">Analysis</p>
              ...
              {([
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 py-2 rounded-lg text-[12px] font-medium transition-all group relative ${
                      isActive
                        ? 'nav-active text-[#c4c4ff]'
                        : soon
                          ? 'px-2.5 text-[#3a3a54] cursor-default'
                          : 'px-2.5 text-[#7070a0] hover:text-[#b0b0d0] hover:bg-white/[0.05]'
                    }`}
                  >
                    <Icon size={14} className={isActive ? 'text-violet-400' : 'text-[#4e4e70] group-hover:text-[#8080b0]'} />
                    <span className="flex-1 text-left truncate">{label}</span>
                    {soon && (
                      <span className="text-[9px] bg-[#1e1e38] text-[#4e4e6a] px-1.5 py-0.5 rounded-md font-medium">Soon</span>
                    )}
                  </button>
                );
              })}
          ...
          {/* DATA */}
          <div>
            <p className="px-2 mb-1.5 text-[9px] font-semibold text-[#4e4e6a] tracking-[0.12em] uppercase">Data</p>
            <div className="space-y-0.5">
              {(() => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 py-2 rounded-lg text-[12px] font-medium transition-all group relative ${
                      isActive ? 'nav-active text-[#c4c4ff]' : 'px-2.5 text-[#7070a0] hover:text-[#b0b0d0] hover:bg-white/[0.05]'
                    }`}
                  >
                    <History size={14} className={isActive ? 'text-violet-400' : 'text-[#4e4e70] group-hover:text-[#8080b0]'} />
                    <span className="flex-1 text-left">Analysis History</span>
                    {count > 0 && (
                      <span className="text-[9px] bg-violet-500/25 text-violet-300 px-1.5 py-0.5 rounded-full font-semibold">{count}</span>
                    )}
                  </button>
                );
              })()}
          ...
          {/* INSTALL */}
          <div>
            <p className="px-2 mb-1.5 text-[9px] font-semibold text-[#4e4e6a] tracking-[0.12em] uppercase">설치</p>
            <div className="space-y-0.5">
              {(() => {
                ...
                return (
                  ...
                  >
                    <Package2 size={14} className={isActive ? 'text-violet-400' : 'text-[#4e4e70] group-hover:text-[#8080b0]'} />
          ...
          {/* SETTINGS */}
          <div>
            <p className="px-2 mb-1.5 text-[9px] font-semibold text-[#4e4e6a] tracking-[0.12em] uppercase">Settings</p>
            <div className="space-y-0.5">
              {[
                { label: 'API 설정', Icon: Settings2 },
                { label: '내보내기 설정', Icon: FileSpreadsheet },
              ].map(({ label, Icon }) => (
                <div key={label} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12px] text-[#3a3a54] cursor-default">
                  <Icon size={14} className="text-[#2a2a44]" />
                  <span>{label}</span>
                  <span className="ml-auto text-[9px] bg-[#1e1e38] text-[#4e4e6a] px-1.5 py-0.5 rounded-md font-medium">Soon</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── 하단 상태 ────────────────────────────────────────────── */}
        <div className="px-4 py-3 border-t border-[#ffffff1a]">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full dot-live shrink-0" />
            <span className="text-[11px] text-[#6b6b8f]">Connected</span>
          </div>
        </div>
      </aside>

      {/* Main Content Area */}
      <main className="flex-1 flex flex-col h-screen overflow-y-auto bg-[#05050e]">
          ...
          {activeTab === 'channel-config' ? (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[22px] font-bold text-white tracking-tight">채널 통합 분석</h2>
                  <p className="text-[13px] text-[#7878a8] mt-1">YouTube 채널 평균 조회수 및 영상 데이터 수집</p>
                </div>
                <button
                  onClick={() => setShowHelp(!showHelp)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${showHelp ? 'bg-violet-500/20 text-violet-300 border border-violet-400/40' : 'bg-white/[0.07] text-[#9494b8] hover:text-[#9494b8]'}`}
                >
                  <Info size={13} /> 가이드
                </button>
              </div>
              ...
              {showHelp && (
                <div className="bg-[#0c0c22] border border-violet-400/30 rounded-xl p-5 animate-in fade-in duration-200">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      <h4 className="font-medium text-violet-400 text-xs flex items-center gap-1.5"><CalendarDays size={12} /> 분석 기간</h4>
                      <p className="text-[#9494b8] text-xs leading-relaxed">수집할 영상의 게시 기간을 필터링합니다. 전체 선택 시 기간 제한 없이 수집합니다.</p>
                    </div>
                    <div className="space-y-1.5">
                      <h4 className="font-medium text-violet-400 text-xs flex items-center gap-1.5"><Activity size={12} /> 수집 개수</h4>
                      <p className="text-[#9494b8] text-xs leading-relaxed">채널당 수집할 최대 영상 수를 지정합니다. 쇼츠/롱폼 각각 설정 가능합니다.</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                <div className="xl:col-span-3">
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4 h-full">
                    <div className="flex items-center justify-between">
                      <label className="text-[12px] font-semibold text-[#a0a0c0] flex items-center gap-1.5">
                        <List size={13} className="text-violet-500" /> Channel List
                        {channelList.length > 0 && <span className="bg-violet-500/20 text-violet-300 px-1.5 py-0.5 rounded text-[10px]">{channelList.length}</span>}
                      </label>
                      {channelList.length > 0 && (
                        <button onClick={clearChannelList} className="text-xs text-[#c8c8e8] hover:text-red-400 transition-colors flex items-center gap-1">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-white/[0.07] border border-[#ffffff1e] rounded-lg px-3 py-2 text-[13px] text-white font-mono placeholder:text-[#404060] focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/20"
                      />
                      <button
                    ...
                    <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
                      {channelList.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-[#c8c8e8] space-y-2">
                          <List size={28} strokeWidth={1} />
                          <p className="text-xs">채널을 추가하세요</p>
                        </div>
                      ) : channelList.map((ch, i) => (
                        <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                          <div className="w-1.5 h-1.5 bg-[#1e1e34] rounded-full shrink-0" />
                          <span className="flex-1 text-xs font-mono text-[#9494b8] truncate">{ch}</span>
                          <button onClick={() => removeChannelItem(i)} className="opacity-0 group-hover:opacity-100 text-[#c8c8e8] hover:text-red-400 transition-all">
                            <X size={13} />
                          </button>
                        </div>
                      ))}
                    ...
                    {channelList.length > 0 && (
                      <p className="text-[10px] text-[#c8c8e8]">{channelList.length}개 채널 · Enter 또는 추가 버튼으로 입력</p>
                    )}
                  </div>
                </div>
                ...
                {/* Right: Options */}
                  <div className="xl:col-span-2 flex flex-col space-y-4">
                    ...
                    <div className="bg-[#0c0c22] p-5 rounded-xl border border-[#ffffff16] space-y-5">
                        <h3 className="text-sm font-medium text-white flex items-center gap-2 pb-2 border-b border-white/[0.11]">
                          <Calendar size={15} className="text-violet-500" /> 분석 기간 설정
                        </h3>

                        <div className="space-y-4">
                           <div className="flex justify-between items-center">
                              <label className="text-xs text-[#c8c8e8] flex items-center gap-1.5">
                                <CalendarDays size={13} className="text-violet-400" /> 전체 영상 기준 기간
                              </label>
                              <button
                                onClick={() => setUseDateFilter(!useDateFilter)}
                                className={`px-2.5 py-0.5 rounded-full text-xs font-medium transition-all ${useDateFilter ? 'bg-violet-600 text-white' : 'bg-white/8 text-[#9494b8]'}`}
                              >
                           ...
                           <div className={`grid grid-cols-4 gap-1.5 transition-opacity ${!useDateFilter ? 'opacity-30' : ''}`}>
                             {(['all', '90d', '30d', '7d'] as AnalysisPeriod[]).map(p => (
                               <button
                                 ...
                                 onClick={() => setPeriod(p)}
                                 className={`py-2 text-xs font-medium rounded-lg transition-all ${period === p ? 'bg-white text-black' : 'bg-white/[0.07] text-[#c8c8e8] hover:bg-white/[0.14] hover:text-white'}`}
                               >
                                 {periodLabels[p]}
                               </button>
                             ))}
                           </div>
                           <p className="text-xs text-[#c8c8e8] text-center">설정한 기간 내의 영상만 수집 대상에 포함됩니다.</p>
                        </div>
                    </div>

                    {/* SECTION 2: 영상 수집 개수 필터 (통합 ENABLED/DISABLED) */}
                    <div className="bg-[#0c0c22] p-5 rounded-xl border border-[#ffffff16] space-y-5">
                        <div className="flex justify-between items-center pb-2 border-b border-white/[0.11]">
                        ...
                        <div className="space-y-3">
                           <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  ...
                                  className={`${(useShorts && useGlobalCountFilter) ? 'text-violet-500' : 'text-[#c8c8e8]'} transition-opacity ${!useGlobalCountFilter ? 'opacity-30' : ''}`}
                                >
                           ...
                           <div className={`space-y-2 transition-opacity ${(!useShorts || !useGlobalCountFilter) ? 'opacity-30' : ''}`}>
                              <div className="flex justify-between text-xs">
                                <span className="text-[#9494b8]">Max Target</span>
                              ...
                              <input
                               ...
                               className="w-full appearance-none bg-white/[0.10] h-1.5 rounded-full accent-violet-500"
                              />
                           </div>
                        </div>
                        ...
                        <div className="space-y-3">
                           <div className="flex justify-between items-center">
                              <label className="text-xs text-[#c8c8e8] flex items-center gap-1.5">
                                <MonitorPlay size={13} className="text-[#c8c8e8]" /> Longform Target
                              </label>
                              <div className="flex items-center gap-3">
                                <button
                                  disabled={!useGlobalCountFilter}
                                  onClick={() => setUseLongs(!useLongs)}
                                  className={`${(useLongs && useGlobalCountFilter) ? 'text-white' : 'text-[#c8c8e8]'} transition-opacity ${!useGlobalCountFilter ? 'opacity-30' : ''}`}
                                >
                           ...
                           <div className={`space-y-2 transition-opacity ${(!useLongs || !useGlobalCountFilter) ? 'opacity-30' : ''}`}>
                              <div className="flex justify-between text-xs">
                                <span className="text-[#9494b8]">Max Target</span>
                                <span className="text-[#9494b8] font-medium">{useGlobalCountFilter ? `${targetLong}개` : '전체 수집'}</span>
                              </div>
                              <input
                               type="range"
                               min="1"
                               ...
                               className="w-full appearance-none bg-white/[0.10] h-1.5 rounded-full accent-zinc-400"
                              />
                           </div>
                        </div>
                    </div>
              ...
              {isProcessing && channelResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-3 animate-in fade-in duration-300">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm font-medium text-white">
                      <Loader2 size={14} className="animate-spin text-violet-400" /> 분석 진행 중
                    </div>
                    <span className="text-xs text-[#9494b8] tabular-nums">{channelDone} / {channelTotal} 완료 · {channelProgress}%</span>
                  </div>
                  ...
                  <div className="space-y-1 max-h-20 overflow-y-auto">
                    {channelResults.filter(r => r.status !== 'pending').slice(-4).map((r, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-[#c8c8e8]">
              ...
              {channelResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <button onClick={() => setShowChannelResults(p => !p)} className="w-full flex items-center justify-between px-6 py-4 hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-2.5">
                      {showChannelResults ? <ChevronDown size={15} className="text-[#9494b8]" /> : <ChevronRight size={15} className="text-[#9494b8]" />}
                      <span className="text-sm font-medium text-white">분석 결과</span>
                      <span className="text-xs text-[#c8c8e8]">{channelResults.filter(r => r.status === 'completed').length}개 완료{channelResults.filter(r => r.status === 'error').length > 0 ? ` · ${channelResults.filter(r => r.status === 'error').length}개 오류` : ''}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  {showChannelResults && (
                    <div className="border-t border-white/[0.11]">
                      <div className="flex items-center justify-between px-6 py-3 border-b border-white/[0.11] bg-[#05050e]/50">
                        <div className="flex gap-1">
                          {(['table','chart','raw'] as ResultTab[]).map(t => (
                            <button key={t} onClick={() => setChannelResultTab(t)} className={`px-3 py-1 rounded text-xs font-medium transition-all ${channelResultTab === t ? 'bg-violet-600 text-white' : 'text-[#9494b8] hover:text-[#9494b8] hover:bg-white/[0.07]'}`}>
                              {t === 'table' ? 'TABLE' : t === 'chart' ? 'CHART' : 'RAW DATA'}
                            </button>
                          ))}
                        </div>
                        <div className="flex gap-1.5">
                          <button onClick={handleDownloadExcel} className="flex items-center gap-1 px-2.5 py-1 bg-white/[0.07] hover:bg-white/[0.14] text-[#c8c8e8] hover:text-white rounded text-xs transition-all"><FileSpreadsheet size={11} /> Excel</button>
                          <button onClick={() => navigator.clipboard.writeText(channelResults.map(r => [r.channelName, r.channelId, r.subscriberCount, r.avgShortsViews, r.avgLongViews].join('\t')).join('\n'))} className="px-2.5 py-1 bg-white/[0.07] hover:bg-white/[0.14] text-[#c8c8e8] hover:text-white rounded text-xs transition-all">Copy</button>
                        </div>
                      </div>
                      {channelResultTab === 'table' && (
                        <div className="overflow-x-auto">
                          <table className="w-full text-left border-collapse">
                            <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                              <tr>
                                <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Channel</th>
                                <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Subscribers</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Shorts Avg</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Longform Avg</th>
                                <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Detail</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[#ffffff0c]">
                              {channelResults.map(r => (
                                <tr key={r.channelId} className="hover:bg-white/[0.05] transition-colors group">
                                  <td className="px-6 py-3.5 flex items-center gap-3">
                                    {r.thumbnail ? <img src={r.thumbnail} className="w-8 h-8 rounded-lg object-cover border border-[#ffffff16] shrink-0" /> : <div className="w-8 h-8 bg-[#0a0a18] rounded-lg flex items-center justify-center shrink-0"><Loader2 className="animate-spin text-[#c8c8e8]" size={13} /></div>}
                                    <div className="min-w-0">
                                      <div className="text-[12px] font-semibold text-[#a0a0c0] group-hover:text-violet-400 transition-colors flex items-center gap-1.5 truncate max-w-[220px]">
                                        {r.channelName}
                                        ...
                                        {r.status === 'pending' && <span className="text-[10px] text-[#c8c8e8] shrink-0">대기</span>}
                                      </div>
                                      <div className="text-[10px] text-[#c8c8e8] font-mono mt-0.5 truncate max-w-[200px]">{r.status === 'error' ? r.error : r.channelId}</div>
                                    </div>
                                  </td>
                                  <td className="px-6 py-3.5 text-center"><span className="bg-white/[0.07] px-2.5 py-1 rounded text-[#c8c8e8] text-xs border border-[#ffffff16]">{r.status === 'completed' ? formatNumber(r.subscriberCount) : '—'}</span></td>
                                  <td className="px-6 py-3.5 text-right"><div className="text-sm font-semibold text-violet-400">{r.avgShortsViews > 0 ? r.avgShortsViews.toLocaleString() : '—'}</div><div className="text-[10px] text-[#c8c8e8] mt-0.5">{r.shortsCountFound > 0 ? `${r.shortsCountFound} Shorts` : ''}</div></td>
                                  <td className="px-6 py-3.5 text-right"><div className="text-sm font-semibold text-[#c8c8e8]">{r.avgLongViews > 0 ? r.avgLongViews.toLocaleString() : '—'}</div><div className="text-[10px] text-[#c8c8e8] mt-0.5">{r.longCountFound > 0 ? `${r.longCountFound} Videos` : ''}</div></td>
                                  <td className="px-6 py-3.5 text-center"><button disabled={r.status !== 'completed'} onClick={() => setSelectedChannel(r)} className="p-1.5 bg-white/[0.07] hover:bg-violet-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"><Eye size={14} /></button></td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                      ...
                      {channelResultTab === 'chart' && (
                        <div className="p-6 space-y-6">
                          {(() => {
                            ...
                            if (!done.length) return <p className="text-xs text-[#c8c8e8] text-center py-8">완료된 채널이 없습니다.</p>;
                            const maxShorts = Math.max(...done.map(r => r.avgShortsViews), 1);
                            const maxLong = Math.max(...done.map(r => r.avgLongViews), 1);
                            return (
                              <>
                                <div>
                                  <p className="text-[12px] font-semibold text-[#a0a0c0] mb-3 flex items-center gap-1.5"><Radio size={12} className="text-violet-400" /> Shorts 평균 조회수</p>
                                  <div className="space-y-2">
                                    {done.map(r => (
                                      <div key={r.channelId} className="flex items-center gap-3">
                                        <span className="text-xs text-[#9494b8] w-28 truncate shrink-0">{r.channelName}</span>
                                        ...
                                        <span className="text-xs text-[#c8c8e8] w-20 text-right shrink-0 tabular-nums">{r.avgShortsViews.toLocaleString()}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                                <div>
                                  <p className="text-[12px] font-semibold text-[#a0a0c0] mb-3 flex items-center gap-1.5"><MonitorPlay size={12} className="text-[#c8c8e8]" /> Longform 평균 조회수</p>
                                  <div className="space-y-2">
                                    {done.map(r => (
                                      <div key={r.channelId} className="flex items-center gap-3">
                                        <span className="text-xs text-[#9494b8] w-28 truncate shrink-0">{r.channelName}</span>
                                        ...
                                        <span className="text-xs text-[#c8c8e8] w-20 text-right shrink-0 tabular-nums">{r.avgLongViews.toLocaleString()}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                      ...
                      {channelResultTab === 'raw' && (
                        <pre className="p-6 text-[11px] text-[#9494b8] overflow-auto max-h-96 font-mono leading-relaxed">{JSON.stringify(channelResults.map(r => ({ channelId: r.channelId, channelName: r.channelName, subscriberCount: r.subscriberCount, avgShortsViews: r.avgShortsViews, avgLongViews: r.avgLongViews, status: r.status })), null, 2)}</pre>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              {/* Header */}
              <div>
                <h2 className="text-[22px] font-bold text-white tracking-tight">단일 영상 분석</h2>
                <p className="text-[13px] text-[#7878a8] mt-1">YouTube 영상 URL 또는 ID로 조회수·댓글 분석</p>
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                {/* Left: list input */}
                <div className="xl:col-span-3 bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-[12px] font-semibold text-[#a0a0c0] flex items-center gap-1.5">
                      <Video size={13} className="text-violet-500" /> Video List
                      {videoList.length > 0 && <span className="bg-violet-500/20 text-violet-300 px-1.5 py-0.5 rounded text-[10px]">{videoList.length}</span>}
                    </label>
                    {videoList.length > 0 && (
                      <button onClick={() => setVideoInput('')} className="text-xs text-[#c8c8e8] hover:text-red-400 transition-colors flex items-center gap-1"><Trash2 size={11} /> 전체 삭제</button>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input
                  ...
                  <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
                    {videoList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-[#c8c8e8] space-y-2"><Video size={26} strokeWidth={1} /><p className="text-xs">영상을 추가하세요</p></div>
                    ) : videoList.map((v, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                        <div className="w-1.5 h-1.5 bg-[#1e1e34] rounded-full shrink-0" />
                        <span className="flex-1 text-xs font-mono text-[#9494b8] truncate">{v}</span>
                        <button onClick={() => removeVideoItem(i)} className="opacity-0 group-hover:opacity-100 text-[#c8c8e8] hover:text-red-400 transition-all"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-3 flex-1">
                    <h3 className="text-[12px] font-semibold text-[#a0a0c0]">수집 설정</h3>
                    <p className="text-xs text-[#c8c8e8] leading-relaxed">URL 또는 11자리 영상 ID를 입력하세요. 중복은 자동으로 제거됩니다.</p>
                    <div className="bg-white/[0.05] rounded-lg p-3 space-y-1">
                      <p className="text-[10px] text-[#c8c8e8]">지원 형식</p>
                      <p className="text-[10px] text-[#c8c8e8] font-mono">youtube.com/watch?v=xxx</p>
                      <p className="text-[10px] text-[#c8c8e8] font-mono">youtu.be/xxx</p>
                      <p className="text-[10px] text-[#c8c8e8] font-mono">youtube.com/shorts/xxx</p>
                    </div>
                  </div>
              ...
              {isProcessing && videoResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-3 animate-in fade-in duration-300">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[#9494b8] tabular-nums">{videoDone} / {videoTotal} 완료 · {videoProgress}%</span>
                  </div>
                  <div className="w-full bg-white/8 rounded-full h-1 overflow-hidden">
              ...
              {videoResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <button onClick={() => setShowVideoResults(p => !p)} className="w-full flex items-center justify-between px-6 py-4 hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-2.5">
                      {showVideoResults ? <ChevronDown size={15} className="text-[#9494b8]" /> : <ChevronRight size={15} className="text-[#9494b8]" />}
                      <span className="text-sm font-medium text-white">수집 결과</span>
                      <span className="text-xs text-[#c8c8e8]">{videoResults.filter(v => v.status === 'completed').length}개 완료</span>
                    </div>
                  ...
                  {showVideoResults && (
                    <div className="border-t border-white/[0.11]">
                      <div className="flex items-center justify-between px-6 py-3 border-b border-white/[0.11] bg-[#05050e]/50">
                        <div className="flex gap-1">
                          {(['table','chart','raw'] as ResultTab[]).map(t => (
                            <button key={t} onClick={() => setVideoResultTab(t)} className={`px-3 py-1 rounded text-xs font-medium transition-all ${videoResultTab === t ? 'bg-violet-600 text-white' : 'text-[#9494b8] hover:text-[#9494b8] hover:bg-white/[0.07]'}`}>
                              {t === 'table' ? 'TABLE' : t === 'chart' ? 'CHART' : 'RAW DATA'}
                            </button>
                          ))}
                        </div>
                      ...
                      {videoResultTab === 'table' && (
                        <div className="overflow-x-auto">
                          <table className="w-full text-left border-collapse">
                            <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                              <tr>
                                <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Video</th>
                                <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Channel</th>
                                <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Likes / Comments</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Views</th>
                                <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Detail</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[#ffffff0c]">
                              {videoResults.map(v => (
                                <tr key={v.videoId} className="hover:bg-white/[0.05] transition-colors group">
                                  <td className="px-6 py-3.5 flex items-center gap-3">
                                    {v.thumbnail ? <img src={v.thumbnail} className={`rounded-lg object-cover border border-[#ffffff16] shrink-0 ${v.isShort ? 'w-7 h-10' : 'w-14 h-9'}`} /> : <div className="w-8 h-8 bg-[#0a0a18] rounded-lg flex items-center justify-center shrink-0"><Loader2 className="animate-spin text-[#c8c8e8]" size={13} /></div>}
                                    <div className="min-w-0">
                                      <div className="text-[12px] font-semibold text-[#a0a0c0] group-hover:text-violet-400 transition-colors truncate max-w-[280px]">{v.title}</div>
                                      <div className="text-[10px] text-[#c8c8e8] font-mono mt-0.5">{v.status === 'error' ? v.error : v.videoId}</div>
                                    </div>
                                  </td>
                                  <td className="px-6 py-4 text-[13px] text-[#c8c8e8]">{v.channelTitle || '—'}</td>
                                  <td className="px-6 py-3.5 text-center">
                                    ...
                                    <div className="text-[10px] text-[#c8c8e8] flex items-center justify-center gap-1 mt-0.5"><MessageSquare size={10} /> {v.commentCount.toLocaleString()}</div>
                                  </td>
                                  ...
                                  <td className="px-6 py-3.5 text-center flex items-center justify-center gap-1.5">
                                    <button disabled={v.status !== 'completed'} onClick={() => setSelectedVideo(v)} className="p-1.5 bg-white/[0.07] hover:bg-white/12 text-[#c8c8e8] hover:text-white rounded-lg transition-all disabled:opacity-20 active:scale-90"><Eye size={13} /></button>
                                    <a href={v.isShort ? `https://youtube.com/shorts/${v.videoId}` : `https://youtube.com/watch?v=${v.videoId}`} target="_blank" className="p-1.5 bg-white/[0.07] hover:bg-violet-600 text-[#c8c8e8] hover:text-white rounded-lg transition-all"><ExternalLink size={13} /></a>
                                  </td>
                                </tr>
                              ))}
                            </tbody>
                      ...
                      {videoResultTab === 'chart' && (
                        <div className="p-6">
                          {(() => {
                            ...
                            if (!done.length) return <p className="text-xs text-[#c8c8e8] text-center py-8">완료된 영상이 없습니다.</p>;
                            const maxViews = Math.max(...done.map(v => v.viewCount), 1);
                            return (
                              <div>
                                <p className="text-[12px] font-semibold text-[#a0a0c0] mb-3 flex items-center gap-1.5"><Eye size={12} className="text-violet-400" /> 조회수 분포</p>
                                <div className="space-y-2">
                                  {done.slice(0, 20).map(v => (
                                    <div key={v.videoId} className="flex items-center gap-3">
                                      <span className="text-xs text-[#c8c8e8] w-36 truncate shrink-0">{v.title}</span>
                                      <div className="flex-1 bg-white/[0.07] rounded-full h-1.5 overflow-hidden">
                                        <div className="h-full bg-violet-500 rounded-full transition-all duration-500" style={{ width: `${(v.viewCount / maxViews) * 100}%` }} />
                                      </div>
                                      <span className="text-xs text-[#c8c8e8] w-20 text-right tabular-nums shrink-0">{v.viewCount.toLocaleString()}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                      ...
                      {videoResultTab === 'raw' && (
                        <pre className="p-6 text-[11px] text-[#9494b8] overflow-auto max-h-96 font-mono leading-relaxed">{JSON.stringify(videoResults.map(v => ({ videoId: v.videoId, title: v.title, viewCount: v.viewCount, likeCount: v.likeCount, commentCount: v.commentCount, status: v.status })), null, 2)}</pre>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              {/* Header */}
              <div>
                <h2 className="text-[22px] font-bold text-white tracking-tight">채널 광고 분석</h2>
                <p className="text-[13px] text-[#7878a8] mt-1">채널별 광고 영상 수 및 광고 비율 분석</p>
              </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                <div className="xl:col-span-3 bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-[12px] font-semibold text-[#a0a0c0] flex items-center gap-1.5">
                      <List size={13} className="text-violet-500" /> Channel List
                      {adList.length > 0 && <span className="bg-violet-500/20 text-violet-300 px-1.5 py-0.5 rounded text-[10px]">{adList.length}</span>}
                    </label>
                    {adList.length > 0 && (
                      <button onClick={clearAdList} className="text-xs text-[#c8c8e8] hover:text-red-400 transition-colors flex items-center gap-1"><Trash2 size={11} /> 전체 삭제</button>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input
                  ...
                  <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
                    {adList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-[#c8c8e8] space-y-2"><List size={26} strokeWidth={1} /><p className="text-xs">채널을 추가하세요</p></div>
                    ) : adList.map((ch, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                        <div className="w-1.5 h-1.5 bg-[#1e1e34] rounded-full shrink-0" />
                        <span className="flex-1 text-xs font-mono text-[#9494b8] truncate">{ch}</span>
                        <button onClick={() => removeAdItem(i)} className="opacity-0 group-hover:opacity-100 text-[#c8c8e8] hover:text-red-400 transition-all"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                  {/* Date filter */}
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                    <div className="flex items-center justify-between pb-2 border-b border-white/[0.11]">
                      ...
                      <button
                        ...
                        className={`px-2.5 py-0.5 rounded-full text-xs font-medium transition-all ${adUseDateFilter ? 'bg-violet-600 text-white' : 'bg-white/8 text-[#9494b8]'}`}
                      >
                    ...
                    <div className={`grid grid-cols-4 gap-1.5 transition-opacity ${!adUseDateFilter ? 'opacity-30 pointer-events-none' : ''}`}>
                      {(['all', '90d', '30d', '7d'] as AnalysisPeriod[]).map(p => (
                        <button key={p} onClick={() => setAdPeriod(p)} className={`py-2 text-xs font-medium rounded-lg transition-all ${adPeriod === p ? 'bg-white text-black' : 'bg-white/[0.07] text-[#c8c8e8] hover:bg-white/[0.14] hover:text-white'}`}>
                          {periodLabels[p]}
                        </button>
                      ))}
                    </div>
              ...
              {isProcessing && adResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-3 animate-in fade-in duration-300">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[#9494b8] tabular-nums">{adDone} / {adTotal} 완료 · {adProgress}%</span>
                  </div>
                  <div className="w-full bg-white/8 rounded-full h-1 overflow-hidden">
              ...
              {adResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <button onClick={() => setShowAdResults(p => !p)} className="w-full flex items-center justify-between px-6 py-4 hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-2.5">
                      {showAdResults ? <ChevronDown size={15} className="text-[#9494b8]" /> : <ChevronRight size={15} className="text-[#9494b8]" />}
                      <span className="text-sm font-medium text-white">광고 분석 결과</span>
                      <span className="text-xs text-[#c8c8e8]">{adResults.filter(r => r.status === 'completed').length}개 완료</span>
                    </div>
                  ...
                  {showAdResults && (
                    <div className="border-t border-white/[0.11]">
                      <div className="flex items-center justify-between px-6 py-3 border-b border-white/[0.11] bg-[#05050e]/50">
                        <div className="flex gap-1">
                          {(['table','chart','raw'] as ResultTab[]).map(t => (
                            <button key={t} onClick={() => setAdResultTab(t)} className={`px-3 py-1 rounded text-xs font-medium transition-all ${adResultTab === t ? 'bg-violet-600 text-white' : 'text-[#9494b8] hover:text-[#9494b8] hover:bg-white/[0.07]'}`}>
                              {t === 'table' ? 'TABLE' : t === 'chart' ? 'CHART' : 'RAW DATA'}
                            </button>
                          ))}
                        </div>
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-left border-collapse">
                            <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                              <tr>
                                <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Channel</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Total Videos</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Ad Videos</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Ad Ratio</th>
                                <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Avg Ad Views</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[#ffffff0c]">
                              {adResults.map(r => (
                                <tr key={r.channelId} className="hover:bg-white/[0.05] transition-colors group">
                                  <td className="px-6 py-3.5 flex items-center gap-3">
                                    {r.thumbnail ? <img src={r.thumbnail} className="w-8 h-8 rounded-lg object-cover border border-[#ffffff16] shrink-0" /> : <div className="w-8 h-8 bg-[#0a0a18] rounded-lg flex items-center justify-center shrink-0"><Loader2 className="animate-spin text-[#c8c8e8]" size={13} /></div>}
                                    <div className="min-w-0">
                                      <div className="text-[12px] font-semibold text-[#a0a0c0] truncate max-w-[200px] group-hover:text-violet-400 transition-colors">{r.channelName}</div>
                                      <div className="text-[10px] text-[#c8c8e8] font-mono mt-0.5 truncate max-w-[180px]">{r.status === 'error' ? <span className="text-red-400">{r.error}</span> : r.channelId}</div>
                                    </div>
                                  </td>
                                  <td className="px-6 py-4 text-right text-[13px] text-[#c8c8e8] tabular-nums">{r.status === 'completed' ? r.totalVideoCount.toLocaleString() : '—'}</td>
                                  <td className="px-6 py-4 text-right text-[13px] text-violet-400 tabular-nums font-medium">{r.status === 'completed' ? r.totalAdCount.toLocaleString() : '—'}</td>
                                  ...
                                  <td className="px-6 py-4 text-right text-[13px] text-[#9494b8] tabular-nums">{r.status === 'completed' && r.avgAdViews > 0 ? r.avgAdViews.toLocaleString() : '—'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ...
                        <div className="p-6 space-y-6">
                          {(() => {
                            ...
                            return (
                              <>
                                <div>
                                  <p className="text-[12px] font-semibold text-[#a0a0c0] mb-3">광고 영상 수</p>
                                  <div className="space-y-2">
                                    {done.map(r => (
                                      <div key={r.channelId} className="flex items-center gap-3">
                                        <span className="text-xs text-[#9494b8] w-28 truncate shrink-0">{r.channelName}</span>
                                        ...
                                        <span className="text-xs text-[#c8c8e8] w-16 text-right shrink-0 tabular-nums">{r.totalAdCount}</span>
                      ...
                      {adResultTab === 'raw' && (
                        <pre className="p-6 text-[11px] text-[#9494b8] overflow-auto max-h-96 font-mono leading-relaxed">{JSON.stringify(adResults.map(r => ({ channelId: r.channelId, channelName: r.channelName, totalVideoCount: r.totalVideoCount, totalAdCount: r.totalAdCount, adRatio: r.adRatio, avgAdViews: r.avgAdViews, status: r.status })), null, 2)}</pre>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[22px] font-bold text-white tracking-tight">라이브 지표 분석</h2>
                  <p className="text-[13px] text-[#7878a8] mt-1">CHZZK / SOOP 방송 시청자 지표 수집 · viewership.softc.one</p>
                </div>
              ...
              {!softcLocalRunning && (
                <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#9494b8] mt-1">
              ...
              {showSoftcInstallModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                  <div className="bg-[#0c0c22] border border-[#ffffff1e] rounded-2xl p-7 w-full max-w-md mx-4 shadow-2xl">
                    <div className="flex items-center justify-between mb-5">
                      <h3 className="text-base font-semibold text-white flex items-center gap-2">
                        <ShieldCheck size={18} className="text-orange-400" />
                        TubeMetric SoftC Scraper 설치
                      </h3>
                      <button onClick={() => { setShowSoftcInstallModal(false); setWaitingForSoftcAgent(false); }} className="text-[#6b6b8f] hover:text-white">
                        <X size={18} />
                      </button>
                    </div>
                    <div className="space-y-4 text-xs text-[#9494b8]">
                      <p>라이브 지표를 PC의 Chrome으로 직접 수집하는 에이전트입니다.</p>
                      <div className="bg-white/[0.07] rounded-lg p-3 space-y-1.5">
                        ...
                        <p className="flex items-center gap-2"><Info size={13} className="text-[#6b6b8f]" /> PC에 Chrome이 설치되어 있어야 합니다</p>
                      </div>
                    </div>
                    <div className="mt-6 space-y-2">
                      ...
                      {(detectOS() === 'macos' || detectOS() === 'other') && (
                        <a
                          ...
                          className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#252540] hover:bg-[#3a3a58] text-white text-sm font-medium rounded-lg transition-colors"
                        >
                          <Download size={15} />
                    ...
                    {waitingForSoftcAgent && (
                      <div className="mt-4 flex items-center gap-2 text-xs text-[#6b6b8f]">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                  <div className="bg-[#0c0c22] border border-[#ffffff1e] rounded-2xl p-7 w-full max-w-md mx-4 shadow-2xl">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <button onClick={() => { setShowInstallModal(false); setWaitingForAgent(false); }} className="text-[#6b6b8f] hover:text-white">
                        <X size={18} />
                      </button>
                    </div>
              ...
              {/* 가이드 토글 */}
              <div className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl overflow-hidden">
                <button
                  onClick={() => setShowSoftcGuide(v => !v)}
                  className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-white/[0.09] transition-colors"
                >
                  <p className="text-[12px] font-semibold text-[#a0a0c0] flex items-center gap-2">
                    <Info size={13} className="text-orange-400" />
                    로컬 에이전트 사용 가이드
                  </p>
                  <span className={`text-[#6b6b8f] transition-transform ${showSoftcGuide ? 'rotate-180' : ''}`}>▾</span>
                </button>

                {showSoftcGuide && (
                  <div className="px-5 pb-5 space-y-3 border-t border-white/[0.11]">
                    <div className="space-y-2 text-xs text-[#c8c8e8] pt-3">
                      ...
                      <div className="bg-white/[0.07] rounded-lg p-3 space-y-1.5 mt-2">
                        ...
                        <p className="flex items-center gap-2"><Info size={12} className="text-[#6b6b8f] shrink-0" /> PC에 Google Chrome이 설치되어 있어야 합니다</p>
                        <p className="flex items-center gap-2"><Info size={12} className="text-[#6b6b8f] shrink-0" /> Windows 시작 시 자동 실행, macOS LaunchAgent 등록</p>
                      </div>
                      <p className="text-[#6b6b8f]">크리에이터 ID 형식: <code className="bg-white/8 px-1 rounded">chzzk:채널ID</code> 또는 <code className="bg-white/8 px-1 rounded">soop:아이디</code></p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-5 space-y-3" style={{display:'none'}}>
                <p className="text-[12px] font-semibold text-[#a0a0c0] flex items-center gap-2"><Tv2 size={13} className="text-orange-500" /> 작동 방식</p>
                <div className="space-y-1.5 text-xs text-[#c8c8e8]">
                ...
                <div className="border-t border-white/[0.11] pt-3 text-[10px] text-[#9494b8]">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                <div className="xl:col-span-3 bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {liveList.length > 0 && (
                      <button onClick={clearLiveList} className="text-xs text-[#9494b8] hover:text-red-400 transition-colors flex items-center gap-1"><Trash2 size={11} /> 전체 삭제</button>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${livePlatform === 'chzzk' ? 'bg-blue-600 text-white' : 'bg-white/[0.07] text-[#9494b8] hover:bg-white/[0.14]'}`}
                    >CHZZK</button>
                    <button
                      onClick={() => setLivePlatform('soop')}
                      className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${livePlatform === 'soop' ? 'bg-purple-600 text-white' : 'bg-white/[0.07] text-[#9494b8] hover:bg-white/[0.14]'}`}
                    >SOOP (아프리카TV)</button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-white/[0.07] border border-[#ffffff1e] rounded-lg px-3 py-2 text-[13px] text-white font-mono placeholder:text-[#404060] focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20"
                    />
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                    {liveList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-[#9494b8] space-y-2"><Tv2 size={26} strokeWidth={1} /><p className="text-xs">크리에이터를 추가하세요</p></div>
                    ) : liveList.map((u, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                        ...
                        <span className="flex-1 text-xs font-mono text-[#c8c8e8] truncate">{u.includes(':') ? u.split(':')[1] : u}</span>
                        <button onClick={() => removeLiveItem(i)} className="opacity-0 group-hover:opacity-100 text-[#9494b8] hover:text-red-400 transition-all"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                  ...
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="group relative bg-white/[0.07] border border-[#ffffff16] hover:border-orange-500/30 rounded-xl p-3 transition-all">
                        <label className="absolute -top-2 left-3 bg-[#0c0c22] px-1.5 text-xs text-[#9494b8] group-hover:text-orange-400">Start</label>
                        <div className="flex items-center gap-2">
                      ...
                      <div className="group relative bg-white/[0.07] border border-[#ffffff16] hover:border-orange-500/30 rounded-xl p-3 transition-all">
                        <label className="absolute -top-2 left-3 bg-[#0c0c22] px-1.5 text-xs text-[#9494b8] group-hover:text-orange-400">End</label>
                        <div className="flex items-center gap-2">
                          <Calendar size={13} className="text-orange-500 shrink-0" />
                          <input
                            type="date"
                  ...
                  {liveJobStatus !== 'idle' && (
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-medium ${
                      ...
                      liveJobStatus === 'done'       ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/20' :
              ...
              {liveResults.length > 0 && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.11]">
                    <div className="flex items-center gap-2.5">
                      <Tv2 size={14} className="text-orange-500" />
                      <span className="text-sm font-medium text-white">수집 결과</span>
                      <span className="text-xs text-[#9494b8]">{liveResults.length}개 크리에이터</span>
                    </div>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                        <tr>
                          <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Creator</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Platform</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">방송 수</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">평균 시청자</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">최고 시청자</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">총 방송시간</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Status</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Detail</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#ffffff0c]">
                        {liveResults.map(r => (
                          <tr key={`${r.platform}-${r.creatorId}`} className="hover:bg-white/[0.05] transition-colors group">
                            <td className="px-6 py-3.5">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <div className="text-xs font-medium text-white group-hover:text-orange-400 transition-colors">{r.creatorId}</div>
                            ...
                            <td className="px-6 py-3.5 text-center">
                              <span className="bg-white/[0.07] px-2.5 py-1 rounded text-[#c8c8e8] text-xs border border-[#ffffff16]">{r.streamCount}</span>
                            </td>
                            <td className="px-6 py-3.5 text-right text-sm font-semibold text-orange-400 tabular-nums">{r.avgViewers.toLocaleString()}</td>
                            <td className="px-6 py-4 text-right text-[13px] text-red-400 tabular-nums font-medium">{r.peakViewers.toLocaleString()}</td>
                            <td className="px-6 py-4 text-right text-[13px] text-[#c8c8e8] tabular-nums">{Math.round(r.totalDurationMin / 60)}시간 {r.totalDurationMin % 60}분</td>
                            <td className="px-6 py-3.5 text-center">
                              <span className={`text-[10px] px-2 py-0.5 rounded ${r.status === 'completed' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/15 text-red-300'}`}>
                            ...
                            <td className="px-6 py-3.5 text-center">
                              <button
                                ...
                                className="p-1.5 bg-white/[0.07] hover:bg-orange-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"
                              >
                                <Eye size={14} />
                              </button>
                            </td>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[22px] font-bold text-white tracking-tight">Instagram 릴스 분석</h2>
                  <p className="text-[13px] text-[#7878a8] mt-1">로컬 서버를 통해 릴스 조회수·좋아요·댓글 수집</p>
                </div>
                {igLocalRunning ? (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                    <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                  <div className="bg-[#0c0c22] border border-[#ffffff1e] rounded-2xl p-7 w-full max-w-md mx-4 shadow-2xl">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <button onClick={() => { setShowInstagramInstallModal(false); setWaitingForInstagramAgent(false); }} className="text-[#6b6b8f] hover:text-white">
                        <X size={18} />
                      </button>
                    </div>
                    <div className="space-y-4 text-xs text-[#9494b8]">
              ...
              {/* 작동 방식 안내 */}
              <div className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-5 space-y-3">
                <p className="text-xs font-medium text-[#9494b8] flex items-center gap-2"><Activity size={13} className="text-violet-500" /> 작동 방식</p>
                {igLocalRunning ? (
                  <div className="space-y-1.5 text-xs text-[#9494b8]">
                    <p>① 아래에서 계정을 입력하고 <strong className="text-[#9494b8]">릴스 수집</strong>을 클릭합니다.</p>
                  ...
                  <div className="space-y-1.5 text-xs text-[#9494b8]">
                    <p>① 아래에서 계정을 입력하고 <strong className="text-[#9494b8]">수집 요청</strong>을 클릭합니다.</p>
                ...
                <div className="border-t border-white/[0.11] pt-3 text-xs space-y-1">
                  {igLocalRunning ? (
                    <p className="text-emerald-400">로컬 에이전트 연결됨 — GitHub 토큰 없이 즉시 수집 가능합니다.</p>
                  ) : (
                    <p className="text-[#9494b8]">로컬 PC에서 실제 Chrome을 실행하므로 봇 감지 없이 공개 계정을 수집합니다.</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                <div className="xl:col-span-3 bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {igList.length > 0 && (
                      <button onClick={clearIgList} className="text-xs text-[#c8c8e8] hover:text-red-400 transition-colors flex items-center gap-1"><Trash2 size={11} /> 전체 삭제</button>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-white/[0.07] border border-[#ffffff1e] rounded-lg px-3 py-2 text-[13px] text-white font-mono placeholder:text-[#404060] focus:outline-none focus:border-pink-500/50 focus:ring-1 focus:ring-pink-500/20"
                    />
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                    {igList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-[#c8c8e8] space-y-2"><Instagram size={26} strokeWidth={1} /><p className="text-xs">계정을 추가하세요</p></div>
                    ) : igList.map((u, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                        <span className="text-pink-600 text-xs shrink-0">@</span>
                        <span className="flex-1 text-xs font-mono text-[#9494b8] truncate">{u}</span>
                        <button onClick={() => removeIgItem(i)} className="opacity-0 group-hover:opacity-100 text-[#c8c8e8] hover:text-red-400 transition-all"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                  ...
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between text-xs">
                        <span className="text-[#9494b8]">계정당 릴스 수</span>
                      ...
                      <input
                        ...
                        className="w-full appearance-none bg-white/[0.10] h-1.5 rounded-full accent-pink-500"
                      />
                      <div className="flex justify-between text-[10px] text-[#c8c8e8]">
                        <span>5개</span><span>50개</span>
                      </div>
                    </div>
                    <p className="text-[10px] text-[#c8c8e8]">최신 릴스부터 수집합니다. 많을수록 시간이 오래 걸립니다.</p>
                    <div className="border-t border-white/[0.11] pt-3 space-y-2">
                      <p className="text-xs text-[#9494b8]">Headless 모드</p>
                      <div className="flex gap-2">
                        <button
                          onClick={() => setIgHeadless(true)}
                          className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${igHeadless ? 'bg-pink-600 text-white' : 'bg-white/[0.07] text-[#6b6b8f] hover:bg-white/[0.14]'}`}
                        >ON</button>
                        <button
                          onClick={() => setIgHeadless(false)}
                          className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${!igHeadless ? 'bg-[#252540] text-white' : 'bg-white/[0.07] text-[#6b6b8f] hover:bg-white/[0.14]'}`}
                        >OFF</button>
                      </div>
                      <p className="text-[10px] text-[#c8c8e8]">OFF 시 브라우저 창이 열림 (좋아요·댓글 수집 불안정 시 사용)</p>
                    </div>
                  </div>
                  ...
                  {igJobStatus !== 'idle' && (
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-medium ${
                      ...
                      igJobStatus === 'done'       ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/20' :
                                                     'bg-red-500/15 text-red-300 border border-red-500/20'
                    }`}>
                ...
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.11]">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-medium text-white">수집 결과</span>
                      <span className="text-xs text-[#c8c8e8]">{igResults.length}개 계정</span>
                    </div>
                    <button onClick={loadIgResults} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.07] hover:bg-white/[0.14] rounded-lg text-xs text-[#c8c8e8] hover:text-white transition-all">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                        <tr>
                          <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Account</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Reels</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Avg Comments</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Avg Likes</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Avg Views</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Scraped At</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Detail</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#ffffff0c]">
                        {igResultsLoading ? (
                          <tr><td colSpan={7} className="py-16 text-center"><Loader2 className="animate-spin mx-auto text-[#c8c8e8]" size={22} /></td></tr>
                        ) : igResults.map(r => (
                          <tr key={r.username} className="hover:bg-white/[0.05] transition-colors group">
                            <td className="px-6 py-3.5">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <div className="text-[12px] font-semibold text-[#a0a0c0] group-hover:text-pink-400 transition-colors">@{r.username}</div>
                                  {r.error && <div className="text-[10px] text-red-400 mt-0.5">{r.error}</div>}
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-3.5 text-center">
                              <span className="bg-white/[0.07] px-2.5 py-1 rounded text-[#c8c8e8] text-xs border border-[#ffffff16]">{r.reelCount}</span>
                            </td>
                            <td className="px-6 py-3.5 text-right text-sm font-semibold text-[#c8c8e8] tabular-nums">{r.avgComments.toLocaleString()}</td>
                            <td className="px-6 py-4 text-right text-[13px] text-violet-400 tabular-nums">{r.avgLikes.toLocaleString()}</td>
                            <td className="px-6 py-3.5 text-right text-sm font-semibold text-pink-400 tabular-nums">{r.avgViews.toLocaleString()}</td>
                            <td className="px-6 py-4 text-center text-[13px] text-[#c8c8e8] font-mono">
                            ...
                            <td className="px-6 py-3.5 text-center">
                              <button
                                ...
                                className="p-1.5 bg-white/[0.07] hover:bg-pink-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"
                              >
                                <Eye size={14} />
                              </button>
                            </td>
              ...
              {igResults.length === 0 && !igResultsLoading && igJobStatus === 'idle' && (
                <button onClick={loadIgResults} className="w-full bg-white/[0.07] hover:bg-white/8 text-[#c8c8e8] hover:text-[#c8c8e8] py-3 rounded-lg text-sm flex items-center justify-center gap-2 transition-all">
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[22px] font-bold text-white tracking-tight">TikTok 영상 분석</h2>
                  <p className="text-[13px] text-[#7878a8] mt-1">클라우드 백엔드(yt-dlp)를 통해 조회수·좋아요·댓글 수집</p>
                </div>
                {isBackendAvailable() ? (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                    <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                  <div className="bg-[#0c0c22] border border-[#ffffff1e] rounded-2xl p-7 w-full max-w-md mx-4 shadow-2xl">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <button onClick={() => { setShowTikTokInstallModal(false); setWaitingForTikTokInstall(false); }} className="text-[#6b6b8f] hover:text-white">
                        <X size={18} />
                      </button>
                    </div>
                    <div className="space-y-4 text-xs text-[#9494b8]">
              ...
              {/* 작동 방식 */}
              <div className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-5 space-y-3">
                <p className="text-xs font-medium text-[#9494b8] flex items-center gap-2"><Activity size={13} className="text-cyan-500" /> 작동 방식</p>
                {tkAgentReady ? (
                  <div className="space-y-1.5 text-xs text-[#9494b8]">
                    <p>① 아래에서 TikTok 계정을 입력하고 <strong className="text-[#9494b8]">수집 시작</strong>을 클릭합니다.</p>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
                ...
                <div className="xl:col-span-3 bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {tkList.length > 0 && (
                      <button onClick={clearTkList} className="text-xs text-[#c8c8e8] hover:text-red-400 transition-colors flex items-center gap-1"><Trash2 size={11} /> 전체 삭제</button>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="@username 또는 username 입력 후 Enter"
                      className="flex-1 bg-white/[0.07] border border-[#ffffff1e] rounded-lg px-3 py-2 text-[13px] text-white font-mono placeholder:text-[#404060] focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20"
                    />
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                    {tkList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 text-[#c8c8e8] space-y-2"><Music size={26} strokeWidth={1} /><p className="text-xs">계정을 추가하세요</p></div>
                    ) : tkList.map((u, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.09] border border-[#ffffff16] rounded-lg px-3 py-2 group transition-colors">
                        <span className="text-cyan-600 text-xs shrink-0">@</span>
                        <span className="flex-1 text-xs font-mono text-[#9494b8] truncate">{u}</span>
                        <button onClick={() => removeTkItem(i)} className="opacity-0 group-hover:opacity-100 text-[#c8c8e8] hover:text-red-400 transition-all"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                  <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] p-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between text-xs">
                        <span className="text-[#9494b8]">계정당 영상 수</span>
                      ...
                      <input
                        ...
                        className="w-full appearance-none bg-white/[0.10] h-1.5 rounded-full accent-cyan-500"
                      />
                      <div className="flex justify-between text-[10px] text-[#c8c8e8]">
                        <span>5개</span><span>50개</span>
                      </div>
                    </div>
                    <p className="text-[10px] text-[#c8c8e8]">최신 영상부터 수집합니다. 많을수록 시간이 오래 걸립니다.</p>
                      ...
                      <div className="border-t border-white/[0.11] pt-3 space-y-2">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => setTkHeadless(true)}
                            className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${tkHeadless ? 'bg-cyan-600 text-white' : 'bg-white/[0.07] text-[#6b6b8f] hover:bg-white/[0.14]'}`}>ON</button>
                          <button onClick={() => setTkHeadless(false)}
                            className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${!tkHeadless ? 'bg-[#252540] text-white' : 'bg-white/[0.07] text-[#6b6b8f] hover:bg-white/[0.14]'}`}>OFF</button>
                        </div>
                        <p className="text-[10px] text-[#c8c8e8]">OFF 시 브라우저 창이 열림 (수집 불안정 시 사용)</p>
                      </div>
                    )}
                  </div>
                  ...
                  {tkJobStatus !== 'idle' && (
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-medium ${
                      ...
                      tkJobStatus === 'done'       ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/20' :
                                                     'bg-red-500/15 text-red-300 border border-red-500/20'
                    }`}>
                ...
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.11]">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-medium text-white">수집 결과</span>
                      <span className="text-xs text-[#c8c8e8]">{tkResults.length}개 계정</span>
                    </div>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-[#0d0d24] text-[#7878a8] text-[11px]">
                        <tr>
                          <th className="px-6 py-3.5 font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Account</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Videos</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Avg Views</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Status</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Scraped At</th>
                          <th className="px-6 py-3.5 text-center font-semibold text-[11px] uppercase tracking-[0.05em] text-[#6b6b8f]">Detail</th>
                      ...
                      <tbody className="divide-y divide-[#ffffff0c]">
                        {tkResultsLoading ? (
                          <tr><td colSpan={6} className="py-16 text-center"><Loader2 className="animate-spin mx-auto text-[#c8c8e8]" size={22} /></td></tr>
                        ) : tkResults.map(r => (
                          <tr key={r.username} className="hover:bg-white/[0.05] transition-colors group">
                            <td className="px-6 py-3.5">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <div className="text-[12px] font-semibold text-[#a0a0c0] group-hover:text-cyan-400 transition-colors">@{r.username}</div>
                                  {r.error && <div className="text-[10px] text-red-400 mt-0.5">{r.error}</div>}
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-3.5 text-center">
                              <span className="bg-white/[0.07] px-2.5 py-1 rounded text-[#c8c8e8] text-xs border border-[#ffffff16]">{r.videoCount}</span>
                            </td>
                            ...
                            <td className="px-6 py-3.5 text-center">
                              <button
                                ...
                                className="p-1.5 bg-white/[0.07] hover:bg-cyan-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"
                              >
                                <Eye size={14} />
                              </button>
                            </td>
            ...
            <div className="space-y-6 animate-in fade-in duration-300 max-w-2xl">
              <div>
                <h2 className="text-[22px] font-bold text-white tracking-tight">로컬 에이전트 설치</h2>
                <p className="text-xs text-[#6b6b8f] mt-1">라이브 지표 · Instagram · TikTok 분석에 필요한 모든 에이전트를 한 번에 설치합니다</p>
              </div>

              {/* 통합 설치 카드 */}
              <div className="bg-gradient-to-br from-violet-600/10 to-blue-600/10 border border-violet-400/30 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#6b6b8f]">Python · 모든 패키지 내장 — 별도 설치 불필요</div>
                  </div>
                  <span className="ml-auto text-[10px] bg-violet-600/30 text-violet-300 px-2 py-0.5 rounded-full border border-violet-400/30">권장</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                  {[
                    ...
                    <div key={label} className={`flex items-center gap-2.5 p-3 rounded-xl border ${bg}`}>
                      ...
                      <div>
                        <div className="text-[12px] font-semibold text-[#a0a0c0]">{label}</div>
                        <div className="text-[10px] text-[#4e4e6a]">port {port}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* 특징 */}
                <div className="grid grid-cols-2 gap-2 mb-6 text-xs text-[#9494b8]">
                  {[
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  {(detectOS() === 'macos' || detectOS() === 'other') && (
                    <a
                      ...
                      className="flex items-center justify-center gap-2 flex-1 py-3 bg-[#1e1e34] hover:bg-[#252540] text-white text-sm font-medium rounded-xl transition-colors"
                    >
                ...
                {waitingForAgent && (
                  <div className="mt-3 flex items-center gap-2 text-xs text-[#6b6b8f]">
              ...
              {/* 연결 상태 */}
              <div className="bg-white/[0.05] border border-[#ffffff16] rounded-xl p-4 space-y-2">
                <p className="text-xs font-medium text-[#9494b8] mb-3">현재 에이전트 연결 상태</p>
                {[
                  ...
                  <div key={port} className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs text-[#9494b8]">
                      <span className={`w-1.5 h-1.5 rounded-full ${ok ? 'bg-emerald-400 animate-pulse' : 'bg-[#252540]'}`} />
                      {label} <span className="text-[#3a3a54]">:{port}</span>
                    </div>
                    <span className={`text-[10px] font-medium ${ok ? 'text-emerald-400' : 'text-[#4e4e6a]'}`}>
                      {ok ? '연결됨' : '미연결'}
                    </span>
                  </div>
                ))}
              ...
              {/* 설치 안내 */}
              <div className="bg-white/[0.05] border border-[#ffffff16] rounded-xl p-4 space-y-2 text-xs text-[#6b6b8f]">
                <p className="font-medium text-[#9494b8]">설치 방법</p>
                <ol className="space-y-1 list-decimal list-inside">
                  <li>위 다운로드 버튼으로 파일을 받습니다</li>
                  <li>다운로드된 파일을 실행하고 <strong className="text-[#9494b8]">다음</strong>을 클릭합니다</li>
                  <li>설치가 완료되면 두 에이전트가 자동으로 시작됩니다</li>
                  <li>이 페이지에서 연결 상태가 <span className="text-emerald-400">연결됨</span>으로 바뀌면 수집 가능합니다</li>
                </ol>
                <p className="text-[#4e4e6a] pt-1">※ PC에 Chrome이 설치되어 있어야 합니다</p>
              </div>
            </div>
            ...
            <div className="space-y-5 animate-in fade-in duration-300">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-[22px] font-bold text-white tracking-tight">데이터 대시보드</h2>
                  <p className="text-[13px] text-[#7878a8] mt-1">수집된 데이터를 한눈에 확인하고 엑셀로 내보내세요</p>
                </div>
                <button
                ...
                {([
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-medium transition-all ${
                      dashboardSubTab === tab.id ? 'bg-white/[0.10] text-white' : 'text-[#9494b8] hover:text-[#9494b8]'
                    }`}
                  >
                ...
                return (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                      ...
                      { label: 'Longform 평균 조회', value: avgL > 0 ? avgL.toLocaleString() : '—', sub: '완료 채널 기준', icon: MonitorPlay, color: 'text-[#c8c8e8]' },
                      { label: '총 수집 영상', value: totalVids.toLocaleString(), sub: '쇼츠 + 롱폼 합계', icon: Video, color: 'text-[#c8c8e8]' },
                    ].map((kpi, i) => (
                      <div key={i} className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-[#9494b8]">{kpi.label}</span>
                          <kpi.icon size={14} className={kpi.color} />
                        </div>
                        <div className={`text-2xl font-semibold ${kpi.color}`}>{kpi.value}</div>
                        <div className="text-xs text-[#c8c8e8]">{kpi.sub}</div>
                      </div>
                    ))}
                  </div>
                );
                ...
                return (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                      ...
                      { label: '총 좋아요', value: totalLikes.toLocaleString(), sub: '수집 영상 합계', icon: ThumbsUp, color: 'text-[#c8c8e8]' },
                      { label: 'Shorts 비율', value: videoResults.length > 0 ? `${Math.round(shortsCount/videoResults.length*100)}%` : '—', sub: `쇼츠 ${shortsCount} / 롱폼 ${videoResults.filter(v=>!v.isShort).length}`, icon: Radio, color: 'text-[#c8c8e8]' },
                    ].map((kpi, i) => (
                      <div key={i} className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-[#9494b8]">{kpi.label}</span>
                ...
                return (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                      ...
                      { label: 'Longform 평균 조회', value: avgL > 0 ? avgL.toLocaleString() : '—', sub: '전체 채널 평균', icon: MonitorPlay, color: 'text-[#c8c8e8]' },
                      { label: '최근 수집일', value: lastDate, sub: '가장 최근 기준', icon: CalendarDays, color: 'text-[#c8c8e8]' },
                    ].map((kpi, i) => (
                      <div key={i} className="bg-[#0c0c22] border border-[#ffffff16] rounded-xl p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-[#9494b8]">{kpi.label}</span>
              ...
              {dashboardSubTab === 'scraper' && (
                <div className="bg-[#0c0c22] rounded-xl border border-[#ffffff16] overflow-hidden">
                  <div className="px-6 py-4 border-b border-white/[0.11] flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Activity size={15} className="text-violet-500" />
                      <span className="font-medium text-white text-sm">로컬 스크래퍼 결과</span>
                      <span className="text-xs text-[#c8c8e8]">from GitHub Raw</span>
                    </div>
                    <button
                      onClick={loadScraperResults}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.07] hover:bg-white/[0.14] rounded-lg text-[12px] font-semibold text-[#a0a0c0] hover:text-white transition-all"
                    >
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-[#ffffff0c]">
                        {scraperResultsLoading ? (
                          <tr><td colSpan={6} className="py-16 text-center"><Loader2 className="animate-spin mx-auto text-[#c8c8e8]" size={24} /></td></tr>
                        ) : scraperResults.length === 0 ? (
                          <tr>
                            <td colSpan={6} className="py-24 text-center">
                              <div className="flex flex-col items-center gap-3 text-[#c8c8e8]">
                          ...
                          scraperResults.map((r) => (
                            <tr key={r.channelId} className="hover:bg-white/[0.05] transition-colors group">
                              <td className="px-6 py-4 flex items-center gap-4">
                                {r.thumbnail ? (
                                  <img src={r.thumbnail} className="w-10 h-10 rounded-lg object-cover border border-[#ffffff16]" />
                                ) : (
                                  <div className="w-10 h-10 bg-[#0a0a18] rounded-lg flex items-center justify-center"><Activity className="text-[#c8c8e8]" size={16} /></div>
                                )}
                                <div>
                                  <div className="font-medium text-white text-sm group-hover:text-violet-400 transition-colors">{r.channelName}</div>
                                  <div className="text-xs text-[#c8c8e8] font-mono mt-0.5">{r.channelId}</div>
                                </div>
                              </td>
                              <td className="px-6 py-4 text-center">
                                <span className="bg-white/[0.07] px-3 py-1 rounded-lg text-[#c8c8e8] text-xs border border-[#ffffff16]">{formatNumber(r.subscriberCount)}</span>
                              </td>
                              <td className="px-6 py-4 text-right">
                                <div className="text-base font-semibold text-violet-400">{r.avgShortsViews.toLocaleString()}</div>
                                <div className="text-[13px] text-[#7878a8] mt-1">{r.shortsCountFound} Shorts</div>
                              </td>
                              <td className="px-6 py-4 text-right">
                                <div className="text-base font-semibold text-[#c8c8e8]">{r.avgLongViews.toLocaleString()}</div>
                                <div className="text-[13px] text-[#7878a8] mt-1">{r.longCountFound} Videos</div>
                              </td>
                              <td className="px-6 py-4 text-center text-xs text-[#9494b8] font-mono">
                                {(r as any).scrapedAt ? new Date((r as any).scrapedAt).toLocaleDateString('ko-KR') : '—'}
                              </td>
                              <td className="px-6 py-4 text-center">
                                <button
                                  onClick={() => setSelectedChannel(r)}
                                  className="p-2 bg-white/[0.07] hover:bg-violet-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all active:scale-90"
                                >
                                  <Eye size={16} />
                                </button>
                              </td>
                              ...
                              <tr key={r.channelId} className="hover:bg-white/[0.05] transition-colors group">
                                <td className="px-6 py-4 flex items-center gap-4">
                                  <div className="relative">
                                    {r.thumbnail ? (
                                      ...
                                      <div className="w-10 h-10 bg-[#0a0a18] rounded-lg flex items-center justify-center">
                                        <Loader2 className="animate-spin text-[#c8c8e8]" size={16} />
                                      </div>
                                    )}
                                  </div>
                                  <div>
                                    <div className="font-medium text-white text-sm group-hover:text-violet-400 transition-colors flex items-center gap-2">
                                      {r.channelName}
                                      {r.status === 'error' && (
                                        <span className="text-xs bg-red-500/15 text-red-400 px-2 py-0.5 rounded border border-red-500/20">Error</span>
                                      )}
                                    </div>
                                    <div className="text-xs text-[#c8c8e8] font-mono mt-0.5 max-w-[200px] truncate">{r.status === 'error' ? r.error : r.channelId}</div>
                                  </div>
                                </td>
                                <td className="px-6 py-4 text-center">
                                  <span className="bg-white/[0.07] px-3 py-1 rounded-lg text-[#c8c8e8] text-xs border border-[#ffffff16]">
                                ...
                                <td className="px-6 py-4 text-center">
                                  <button
                                    ...
                                    onClick={() => setSelectedChannel(r)}
                                    className="p-2 bg-white/[0.07] hover:bg-violet-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"
                                  >
                                    <Eye size={16} />
                                  </button>
                                </td>
                              ...
                              <tr key={v.videoId} className="hover:bg-white/[0.05] transition-colors group">
                                <td className="px-6 py-4 flex items-center gap-4">
                                  <div className="relative shrink-0">
                                    {v.thumbnail ? (
                                      <img src={v.thumbnail} className={`rounded-lg object-cover border border-[#ffffff16] ${v.isShort ? 'w-8 h-12' : 'w-16 h-10'}`} />
                                    ) : (
                                      <div className="w-10 h-10 bg-[#0a0a18] rounded-lg flex items-center justify-center">
                                        <Loader2 className="animate-spin text-[#c8c8e8]" size={16} />
                                      </div>
                                  ...
                                  <div className="min-w-0">
                                    <div className="font-medium text-white text-sm group-hover:text-violet-400 transition-colors truncate max-w-[300px]">{v.title}</div>
                                    <div className="text-xs text-[#c8c8e8] font-mono mt-0.5">{v.status === 'error' ? v.error : v.videoId}</div>
                                  </div>
                                </td>
                                <td className="px-6 py-4">
                                  <div className="text-sm text-[#c8c8e8]">{v.channelTitle || '...'}</div>
                                </td>
                                <td className="px-6 py-4 text-center">
                                  <div className="flex flex-col items-center gap-1">
                                    ...
                                    <span className="text-xs text-[#c8c8e8] flex items-center gap-1">
                                ...
                                <td className="px-6 py-4 text-center flex items-center justify-center gap-2">
                                  <button
                                    ...
                                    className="p-2 bg-white/[0.07] hover:bg-white/12 hover:text-white text-[#c8c8e8] rounded-lg transition-all disabled:opacity-20 active:scale-90"
                                  >
                                    <Eye size={16} />
                                  </button>
                                  <a
                                    ...
                                    target="_blank"
                                    className="inline-block p-2 bg-white/[0.07] hover:bg-violet-600 hover:text-white text-[#c8c8e8] rounded-lg transition-all active:scale-90"
                                  >
